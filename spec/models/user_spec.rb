...
describe User do
  ...
  describe "micropost associations" do
    ...
    before(:each) do
      @user = User.create!(@attr)
      @mp1 = Factory(:micropost, :user => @user, :created_at => 1.day.ago)  # Factory girl bypasses attr_accesible
      @mp2 = Factory(:micropost, :user => @user, :created_at => 1.hour.ago)
    end
    
    it "should have a microposts attriutes" do
    ...
    it "should destroy associated microposts" do
      ...
      [@mp1, @mp2].each do |micropost|
        lambda do
          Micropost.find(micropost.id)  # you can omit id as it is often in rails
          # above, it was find_by_id, but now changed to find, so that it can raise error as u can see below
